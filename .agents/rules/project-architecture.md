...
- `src/main/repository-service.ts` composes repository loading, previews, saving, and worktree opening.
- `src/main/repository-loader.ts`, `repository-tree.ts`, `repository-preview.ts`, `repository-worktree.ts`, and `git-service.ts` handle Git metadata, tree construction, preview generation, worktrees, and lower-level git commands.
...
- Opening repositories, switching refs, opening worktrees, opening recent files, and restoring window state can all affect `repoPath`, `rootPath`, `activeRef`, and `source`.
...
- Git ref previews and working-tree previews have different editability. `git-ref` is generally read-only; working tree/worktree sources can be saved.
